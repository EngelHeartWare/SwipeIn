# Swipe In

A native iOS application built with SwiftUI that allows users to track activities with a simple swipe gesture.

[![Platform](https://img.shields.io/badge/Platform-iOS-blue.svg)](https://developer.apple.com/ios/)
[![Swift](https://img.shields.io/badge/Swift-6.2-orange.svg)](https://swift.org)

## ✨ Features
* **Feature 1:** Intuitive Swipe gestures
* **Feature 2:** Minimalistic design
* **Feature 3:** On-deviec data storage

## 📱 Screenshots
<img width="1320" height="2868" alt="screenshot_eng_1" src="https://github.com/user-attachments/assets/99d015e0-366c-42d8-b7c6-a340bc78954c" />
<img width="1320" height="2868" alt="screenshot_eng_2" src="https://github.com/user-attachments/assets/3d73d4a7-4f3a-4acf-8c71-cad4d7b37269" />
<img width="1320" height="2868" alt="screenshot_eng_3" src="https://github.com/user-attachments/assets/0397febb-5734-444f-8465-5cb4fb0fc5ed" />

## 🛠 Requirements
* iOS [16.0]+
* Xcode [26.0.1]+
* Swift [6.2]+

## 🤝 Support the Project
This app is developed as an indie project. If you find the code helpful or enjoy using the app, consider supporting its continued development!

☕️ **[Buy me a coffee!](https://ko-fi.com/engelheartware)**

## 📄 License

**All Rights Reserved**

The source code in this repository is made available for educational and portfolio purposes only. 

Copyright © 2026 Moritz Engelhardt. All rights reserved. 

You are welcome to read the code to learn from it, but you may not use, copy, modify, merge, publish, distribute, sublicense, or sell copies of this software without explicit written permission.
